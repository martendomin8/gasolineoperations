...
export interface ResolvedPort {
  ...
  departureSource: "manual" | "inferred" | null;
...
export interface VoyageState {
  phase: "pre_voyage" | "at_port" | "sailing" | "completed";
...
export interface SchematicBarProps {
...
function shortDate(formatted: string): string {
  // formatInPortTime returns e.g. "30 Apr 10:53". Header wants "30 Apr".
  return formatted.split(/\s+/).slice(0, 2).join(" ");
}
...
function laycanMarginHours(eta: Date | null, laycanRange: string): number | null {
  ...
  // same month/year as ETA; if parsing fails, return null and render "—".
  if (!eta) return null;
  ...
  // End of laycan = last day at 23:59:59 UTC
...
export default function VoyageSchematicBar({
  ...
  // Marker position. Clamp 2…98% so it never sits on top of an endpoint node.
  ...
  return (
    <section className="relative w-full bg-[var(--color-surface-1)] border border-[var(--color-border-subtle)] px-10 pt-8 pb-7">
      {/* Header */}
      <header className="grid grid-cols-[1fr_auto] items-end gap-5 mb-6">
        ...
        >
          ...
          {!marginIsBlown && state.phase !== "pre_voyage" && " · ON LAYCAN"}
...
function Metric({
  ...
  return (
    <div
      className={clsx(
        "px-4.5 first:pl-0 border-r border-[var(--color-border-subtle)]",
