...
Port-vs-incoterm rule: when a port follows an incoterm inline (e.g. "CIF Amsterdam", "FOB Lavera", "DAP Houston", "CFR Lagos"), the port role is determined by the incoterm. For FOB the inline port is the loadport. For CIF / CFR / DAP the inline port is the discharge_port (the destination we deliver to) — NEVER put it in loadport. Only assign loadport from context that explicitly marks loading (e.g. "Loadport: X", "loading ex X", "lifting from X"). If no loadport is explicitly given for a CIF/CFR/DAP recap, leave loadport null rather than guessing.
